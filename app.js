  ...
  function openPersonModal(kind, person) {
    ...
    openModal(`
      ...
      qs('#person-form', modal).addEventListener('submit', async (e) => {
        ...
        if (!isSupplier) {
          // customer.balance is a derived cache — sync.js recomputes it on
          // every sync as balanceBaseline + sum(this customer's sales), and
          // will silently overwrite whatever's typed into the balance field
          // here unless balanceBaseline is kept in lock-step with it. So a
          // manually-entered opening balance (e.g. debt from before this app
          // was used) must be expressed as baseline + existing sales, not as
          // a bare balance value, or it gets wiped the next time a synced
          // sale for this customer triggers a recompute.
          const existingSales = isEdit ? await db.sales.where('customerId').equals(p.id).toArray() : [];
          const salesSum = existingSales.reduce((s, x) => s + (x.total || 0), 0);
          data.balanceBaseline = data.balance - salesSum;
        }
